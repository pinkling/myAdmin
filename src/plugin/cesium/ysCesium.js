...
YsCesium.prototype.delCircleScan = function(circleScan) {
...
YsCesium.prototype.delRadarScan = function(target) {
  this.viewer.scene.globe.depthTestAgainstTerrain = false
...
YsCesium.prototype.addDoubleCircleRipple = function(data) {
  let r1 = data.minR; let r2 = data.minR
  const viewer = this.viewer
  function changeR1() {
...
YsCesium.prototype.addParabola = function(data) {
  ...
  const cities = data.points// 可以为多组哦！
  if (data.flowing) {
    initPolylineTrailLinkMaterialProperty(data)
    data.options.polyline.material = new Cesium.PolylineTrailLinkMaterialProperty(data.options.polyline.material, data.options.polyline.interval)
  }
  for (let j = 0; j < cities.length; j++) {
    const points = parabolaEquation({ pt1: center, pt2: cities[j], height: data.height, num: 100 })
    const pointArr = []
...
YsCesium.prototype.addFlyLinesByCZML = function(data) {
  ...
  const center = data.center; const cities = data.points
  const dsArr = []
  for (let j = 0; j < cities.length; j++) {
    const czml = [
      {
...
YsCesium.prototype.Css3Renderer = function(app, elements, isBackHide) {
    ...
    scene.preRender.addEventListener(function() {
      for (let i = 0; i < container.children.length; i++) {
        ...
        if (Cesium.defined(canvasPosition)) {
          ...
          if (that.isBackHide) {
            const j = camera.position; let n = scene.globe.ellipsoid.cartesianToCartographic(j).height
  ...
  this.__proto__.addEntityLayer = function(object) {
    ...
    // 构建entity
    let height = object.boxHeight; const heightMax = object.boxHeightMax; const heightDif = object.boxHeightDif
    let goflog = true
    // 添加正方体
  ...
  (function(e) {
    ...
    const i = (function() {
      ...
      i = [{
        ...
        value: function(t, n) {
          ...
          t.eachSeries(function(e) {
            e.get('coordinateSystem') === 'GLMap' && (e.coordinateSystem = r)
          })
        }
      },
      {
      ...
      },
      {
        key: 'pointToData',
        value: function(e) {
          const t = this._mapOffset
          const n = viewer.scene.globe.ellipsoid
          const r = new Cesium.cartesian3(e[1] + t, e[2] + t[2], 0)
          const i = n.cartesianToCartographic(r)
          return [i.lng, i.lat]
        }
      },
      {
