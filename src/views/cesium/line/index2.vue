<template>
  <div id="cesiumContainer"></div>
</template>

<script>
let viewer = null
const line = {}
import { default as PolylineTrailMaterial } from '../../../plugin/cesium/PolylineTrailMaterial'
export default {
  name: 'Index',
  data() {
    return {
      viewer: null
    }
  },
  mounted() {
    this.init()
    this.addClick()
    this.addLine()
    this.addLine2()
    this.addLine3()
    this.addLine4()
    this.addLine5()
  },
  methods: {
    init() {
      viewer = new this.Cesium.Viewer('cesiumContainer', {
        terrainProvider: new this.Cesium.CesiumTerrainProvider({ // 添加地形
          url: this.Cesium.IonResource.fromAssetId(1),
          requestWaterMask: true,
          requestVertexNormals: true
        }),
        geocoder: false, // 地理位置查询定位控件
        homeButton: false, // 默认相机位置控件
        timeline: false, // 时间滚动条控件
        navigationHelpButton: false, // 默认的相机控制提示控件
        fullscreenButton: false, // 全屏控件
        scene3DOnly: true, // 每个几何实例仅以3D渲染以节省GPU内存
        baseLayerPicker: false, // 底图切换控件
        CreditsDisplay: false,
        animation: false // 控制场景动画的播放速度控件
      })
      viewer._cesiumWidget._creditContainer.style.display = 'none'

      viewer.scene.camera.setView({
        destination: this.Cesium.Cartesian3.fromDegrees(-122.3472, 47.598, 370),
        orientation: {
          heading: this.Cesium.Math.toRadians(10),
          pitch: this.Cesium.Math.toRadians(-10)
        }
      })
    },
    addClick() {
      const handler = new this.Cesium.ScreenSpaceEventHandler(viewer.scene.canvas)

      handler.setInputAction((movement) => {
        const pick = viewer.scene.pick(movement.position)
        if (this.Cesium.defined(pick) && (pick.id.click === true)) {
          alert(JSON.stringify(pick.id.name))
          console.log('Pick>>', pick)
        }
      }, this.Cesium.ScreenSpaceEventType.LEFT_CLICK)
    },
    addLine() {
      const positions = this.Cesium.Cartesian3.fromDegreesArrayHeights([
        -122.3612, 47.658, 0.0,
        -122.3722, 47.668, 0.0,
        -122.3832, 47.670, 0.0,
        -122.3942, 47.677, 0.0,
        -122.3992, 47.685, 0.0
      ])
      // 流动的线
      line.trailLine = viewer.entities.add({
        name: '动态线',
        polyline: {
          width: 5,
          positions: positions,
          // material: new this.Cesium.PolylineTrailMaterialProperty({ // 尾迹线材质
          material: new PolylineTrailMaterial({ // 尾迹线材质
            color: this.Cesium.Color.fromCssColorString('rgba(90,90,255, 1)'),
            duration: 6800,
            trailLength: 1
          })
        },
        click: true,
        data: {}
      })
    },
    addLine2() {
      const positions = this.Cesium.Cartesian3.fromDegreesArrayHeights([
        -122.3612, 47.658, 0.0,
        -122.2722, 47.638, 0.0
      ])
      // 普通的线
      line.line = viewer.entities.add({
        name: '普通线',
        polyline: {
          width: 5,
          positions: positions,
          clampToGround: true, // 贴地
          material: this.Cesium.Color.RED
        },
        click: true,
        data: {}
      })
    },
    addLine3() {
      const positions = this.Cesium.Cartesian3.fromDegreesArrayHeights([
        -122.3512, 47.648, 0.0,
        -122.2622, 47.628, 0.0
      ])
      // 发光的线
      line.GlowLine = viewer.entities.add({
        name: '发光线',
        polyline: {
          width: 5,
          positions: positions,
          clampToGround: true, // 贴地
          material: new this.Cesium.PolylineGlowMaterialProperty({ // 发光线
            glowPower: 0.2,
            color: this.Cesium.Color.BLUE
          })
        },
        click: true,
        data: {}
      })
    },
    addLine4() {
      const positions = this.Cesium.Cartesian3.fromDegreesArrayHeights([
        -122.3412, 47.638, 0.0,
        -122.2522, 47.618, 0.0
      ])
      // 发光的线
      line.GlowLine = viewer.entities.add({
        name: '箭头线',
        polyline: {
          width: 5,
          positions: positions,
          clampToGround: true, // 贴地
          material: new this.Cesium.PolylineArrowMaterialProperty(this.Cesium.Color.PURPLE) // 箭头线
        },
        click: true,
        data: {}
      })
    },
    addLine5() {
      const positions = this.Cesium.Cartesian3.fromDegreesArrayHeights([
        -122.3312, 47.628, 0.0,
        -122.2422, 47.608, 0.0
      ])
      // 发光的线
      line.dash = viewer.entities.add({
        name: '虚线',
        polyline: {
          width: 5,
          positions: positions,
          clampToGround: true, // 贴地
          material: new this.Cesium.PolylineDashMaterialProperty({ // 虚线
            color: this.Cesium.Color.CYAN
          })
        },
        click: true,
        data: {}
      })
    }
  }
}
</script>

<style scoped>
  #cesiumContainer {
    width: 100%;
    height: calc(100vh - 84px);
  }
</style>
